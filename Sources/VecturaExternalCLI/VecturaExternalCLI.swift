...
struct VecturaExternalCLI: AsyncParsableCommand {
    ...
    static let configuration = CommandConfiguration(
        commandName: "vectura",
        abstract: "A CLI tool for VecturaKitLite vector database using pre-computed embeddings",
        subcommands: [Add.self, Search.self, Delete.self, Reset.self, Mock.self]
    )

    static func setupDB(dbName: String, dimension: Int, numResults: Int, threshold: Float)
        async throws
        -> VecturaExternalKit
    {
...
extension VecturaExternalCLI {
    struct Add: AsyncParsableCommand {
        static let configuration = CommandConfiguration(
            abstract: "Add documents with pre-computed embeddings")
        ...
        func run() async throws {
            ...
            // Parse embedding
            let embeddingValues = embedding.components(separatedBy: ",").compactMap {
                Float($0.trimmingCharacters(in: .whitespaces))
            }

            guard embeddingValues.count == dimension else {
                throw ValidationError(
                    "Embedding must have exactly \(dimension) values, got \(embeddingValues.count)")
            }
    ...
    struct Search: AsyncParsableCommand {
        static let configuration = CommandConfiguration(
            abstract: "Search using a pre-computed query embedding")
        ...
        func run() async throws {
            ...
            guard embeddingValues.count == dimension else {
                throw ValidationError(
                    "Query embedding must have exactly \(dimension) values, got \(embeddingValues.count)"
                )
            }
    ...
    struct Delete: AsyncParsableCommand {
        static let configuration = CommandConfiguration(
            abstract: "Delete documents by metadata filter")
    ...
    struct Reset: AsyncParsableCommand {
        static let configuration = CommandConfiguration(
            abstract: "Reset the database (delete all documents)")
    ...
    struct Mock: AsyncParsableCommand {
        static let configuration = CommandConfiguration(
            abstract: "Add mock data with random embeddings for testing")
        ...
        func run() async throws {
            ...
            print(
                "Example search: vectura-external search --embedding=\""
                    + (0..<dimension).map { _ in "0.1" }.joined(separator: ",") + "\"")
        }
    }
}
